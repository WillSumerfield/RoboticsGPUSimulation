...
import isaacsim.core.utils.rotation as rotation_utils
...
from .grasp_rotate_cfg import GraspRotateEnvCfg


class GraspRotateEnv(DirectRLEnv):
    cfg: GraspRotateEnvCfg

    def __init__(self, cfg: GraspRotateEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Check the object y rotation
        object_rot = rotation_utils.quat_to_euler_angles(self.object.data.body_com_pose_w[:, 3:7])
        rotation_reached = np.abs(object_rot[:, 1] - self.target_rot) < np.pi / 18  # 10 degrees tolerance
        time_out = self.episode_length_buf >= self.max_episode_length - 1
        out_of_bounds = compute_boundaries(self.object_pos[:, 0, :], self.scene.env_origins)
        return (out_of_bounds | rotation_reached), time_out
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # The target rotation
        self.target_rot = np.random.uniform(0, 1, size=len(env_ids))

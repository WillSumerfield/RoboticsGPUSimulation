...
from .grasp_pot_cfg import GraspPotEnvCfg


class GraspPotEnv(DirectRLEnv):
    cfg: GraspPotEnvCfg

    def __init__(self, cfg: GraspPotEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _setup_scene(self):
        ...
        self.handle1 = RigidObject(self.cfg.handle1_cfg)
        self.handle2 = RigidObject(self.cfg.handle2_cfg)
        ...
        self.scene.rigid_objects["handle1"] = RigidObject(self.cfg.handle1_cfg)
        self.scene.rigid_objects["handle2"] = RigidObject(self.cfg.handle2_cfg)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        handle1_root_state = self.handle1.data.default_root_state[env_ids]
        handle1_root_state[:, :3] += self.scene.env_origins[env_ids]
        handle2_root_state = self.handle2.data.default_root_state[env_ids]
        handle2_root_state[:, :3] += self.scene.env_origins[env_ids]
        ...
        self.handle1.write_root_pose_to_sim(handle1_root_state[:, :7], env_ids)
        self.handle2.write_root_pose_to_sim(handle2_root_state[:, :7], env_ids)
        self.object.write_root_velocity_to_sim(object_root_state[:, 7:], env_ids)
        self.handle1.write_root_velocity_to_sim(handle1_root_state[:, 7:], env_ids)
        self.handle2.write_root_velocity_to_sim(handle2_root_state[:, 7:], env_ids)
